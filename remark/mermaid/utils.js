...
const mmdc = ({ configPath, mmdPath, rawSvgPath, svgPath }) => {
  try {
    ...
    // We retain this non-SVGOed copy for debuggability.
    const rawSvg = fs.readFileSync(rawSvgPath, 'utf-8');

    // Process the raw SVG with SVGO.
    ...
    // Write the processed SVG to the intended output path.
    fs.writeFileSync(svgPath, data);
  } catch (err) {
    // These errors tend to get swallowed 🙈.
    // Printing that something has gone wrong is better than nothing.
...
function render(mmdData, rootDir) {
  ...
  const { mmdPath, rawSvgPath, svgNodeUrl, svgPath } = generateFilePaths(
    rootDir,
    [configData, mmdData],
  );
  ...
  // Invoke mermaid CLI
  mmdc({ configPath, mmdPath, rawSvgPath, svgPath });
...
function renderFromFile(mmdPath, rootDir) {
  ...
  const { rawSvgPath, svgNodeUrl, svgPath } = generateFilePaths(rootDir, [
    configData,
    mmdData,
  ]);
